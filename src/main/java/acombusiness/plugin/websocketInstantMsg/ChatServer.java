package acombusiness.plugin.websocketInstantMsg;
...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("Helloadminqq651521191")){
			this.userjoin(message.replaceFirst("Helloadminqq651521191", ""),conn);
		}if(null != message && message.startsWith("LeaveHelloadminqq651521191")){
			this.userLeave(conn);
		}if(null != message && message.contains("helloadmin996")){
			String toUser = message.substring(message.indexOf("helloadmin996")+10, message.indexOf("hellohelloadmin998"));
			message = message.substring(0, message.indexOf("helloadmin996")) +"[私信]  "+ message.substring(message.indexOf("hellohelloadmin998")+12, message.length());
