...
public class StringUtil extends org.apache.commons.lang.StringUtils {
	...
	public static boolean isEmpty(String str) {
		if (str == null || str.length() == 0 || StringUtil.BLANK.equals(StringUtil.trim_(str)) || "undefined".equalsIgnoreCase(StringUtil.trim_(str))
	...
	* @throws
	...
	public static String concatWithDelimiter(Collection c, String delimiter, boolean beginWithDelimiter, boolean endWithDelimiter, String defaultValue) {
		...
		return StringUtil.concatWithDelimiter(c.toArray(), delimiter, beginWithDelimiter, endWithDelimiter, defaultValue);
	}
	...
	public static String timestampToHHMMSS(BigDecimal _timestamp) {
		...
		return (isNegative ? "负" : "") + (hh.compareTo(BigDecimal.ZERO) > 0 ? hh + "时" : "") + (mm.compareTo(BigDecimal.ZERO) > 0 ? mm + "分" : "")
				+ _timestamp.subtract(hh.multiply(new BigDecimal("3600"))).subtract(mm.multiply(new BigDecimal("60"))) + "秒";
	}
		...
		if (StringUtil.isNotEmpty(nickName)) {
			try {
				if (nickName.getBytes().length <= 4) {
					return nickName.getBytes().length != nickName.length() ? (nickName.substring(0, 1) + "***") : (nickName.substring(0, 2) + "***" + (nickName.substring(2)));
				}
				return nickName.getBytes().length != nickName.length() ? (nickName.substring(0, 1) + "***" + nickName.substring(nickName.length() - 1))
						: (nickName.substring(0, 2) + "***" + nickName.substring(nickName.length() - 2));
			} catch (Exception e) {
				return "******";
			}
     ...
     */  
    public static boolean containsAnyOtherStr(String str, String searchChars) {
